...
import com.icix.Locators.TwoActorRequestLocators;
import com.icix.Utilities.CommonLocators;
import com.icix.Utilities.CommonMethods;
import com.icix.Utilities.ReadTestData;
...
public class TwoActorRequestLogics {
	...
	public static void openReqPage() throws Exception {
		...
		// $(CommonLocators.ReqLink).shouldHave(text("Requests"));
	...
	public static void selectDueDate() {
		new WebDriverRunner();
		WebElement ele = WebDriverRunner.getWebDriver().findElement(TwoActorRequestLocators.calendarField);
		Actions act = new Actions(WebDriverRunner.getWebDriver());
		act.moveToElement(ele).build().perform();
		$(TwoActorRequestLocators.calendarArea).waitUntil(Condition.appear, waitFor);
		$(By.xpath("//span[@class='slds-day ng-binding ng-scope'][text()='31']")).waitUntil(Condition.visible, waitFor)
				.click();
	}
